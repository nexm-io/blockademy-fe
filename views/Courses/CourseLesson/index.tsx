        ...
        <div key={course.id} className={`flex gap-4 items-center `}>
          <div className="w-[40px] h-[40px]">
            <Image
              alt="check-icon"
              src={check}
              className={`w-full h-full object-cover flex-shrink-0 rounded-full ${course.is_completed === 1 ? 'bg-blue-100' : ' '}`}
            ></Image>
          </div>
          <div className={`bg-gray-200 cursor-pointer flex md:items-center items-start justify-between rounded-lg flex-1 min-h-[64px] py-5 px-4 gap-5`}>
            <div className="flex md:flex-row flex-col gap-3 md:gap-0 flex-1">
              ...
              <div className=" flex items-center gap-[10px] basis-[20%] line-clamp-1">
                ...
                <span className="line-clamp-1 text-base">{secondsToMinutes(course.duration)} Min</span>
              </div>
            </div>
